...
def _self_test() -> int:
    ...
    # LOCKBOT runs on Windows, where "/tmp" does not exist -- this used a
    # hardcoded Unix path and was the only self-test in the project that
    # could not run on the machine it ships on. A temporary directory is
    # both portable and self-cleaning, so the test never writes next to
    # the real universe.csv it is meant to leave alone.
    import tempfile

    with tempfile.TemporaryDirectory() as folder:
        tmp = os.path.join(folder, "universe_selftest.csv")
        absent = os.path.join(folder, "does_not_exist.csv")
        ...
        check("missing file returns empty", load_universe(absent) == [])

        print("Freshness:")
        check("missing file needs refresh", needs_refresh(absent))
