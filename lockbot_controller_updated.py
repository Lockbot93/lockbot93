...
def run_controller() -> None:
    ...
    while True:
        ...
        scanner_ok = run_component(
            SCANNER_FILE,
            "Market Scanner",
        )

        scanner_state_ok = False

        if scanner_ok:
            scanner_state_ok = display_scanner_state()
        else:
            write_log(
                "Scanner state was not checked because "
                "the Market Scanner failed."
            )

        manager_ok = run_component(
            TRADE_MANAGER_FILE,
            "Trade Manager",
        )
        ...
        if cycle_healthy:
            ...
            write_log(
                ...
                f"System status: {state.system_status}. "
            ...
            write_log(
                ...
                f"Scanner: {scanner_ok}. "
                f"Scanner state: {scanner_state_ok}. "
