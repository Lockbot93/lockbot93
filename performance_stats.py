"""LOCKBOT performance statistics v0.1.

Reads completed trades from lockbot_trade_journal.csv and calculates
portfolio-level performance metrics.

This module does not submit, modify, or cancel brokerage orders.
"""
...
def _to_float(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            f"Invalid {field_name!r} value on journal "
...
def load_completed_trades(
    ...
    with journal_file.open(
        ...
        if missing_columns:
            ...
            raise ValueError(
                "Trade journal is missing required "
...
def calculate_performance_stats(
    ...
    # Stored as a positive dollar amount for reporting.
    ...
    average_loss = (
        sum(losing_values) / losing_trades
        if losing_trades
        else 0.0
    )
    ...
    average_return_percent = (
        sum(return_values) / total_trades
    )

    average_holding_minutes = (
        sum(holding_values) / total_trades
    )

    (
        longest_winning_streak,
        longest_losing_streak,
    ) = _calculate_streaks(pnl_values)

    starting_equity = float(
        trades[0]["account_equity_at_entry"]
    )

    estimated_current_equity = (
        starting_equity + net_profit
    )
    ...
    return PerformanceStats(
        ...
        estimated_current_equity=round(
            estimated_current_equity,
            2,
        ),
    )
...
def print_performance_report(
    ...
    print("=" * 50)
    print("       LOCKBOT PERFORMANCE STATS v0.1")
    print("=" * 50)
    print(
        f"Journal File          : {JOURNAL_FILE.name}"
    )
    print(
    ...
    print(
        f"Gross Profit          : "
        f"{_format_money(stats.gross_profit)}"
    )
    print(
        f"Gross Loss            : "
    ...
    print(
        f"Net Profit            : "
        f"{_format_money(stats.net_profit)}"
    )
    print(
        f"Average Winner        : "
        f"{_format_money(stats.average_win)}"
    )
    print(
        f"Average Loser         : "
        f"{_format_money(stats.average_loss)}"
    )
    print(
        f"Largest Winner        : "
        f"{_format_money(stats.largest_win)}"
    )
    print(
        f"Largest Loser         : "
        f"{_format_money(stats.largest_loss)}"
    )
    print(
        f"Profit Factor         : "
        f"{_format_profit_factor(stats.profit_factor)}"
    )
    print(
        f"Expectancy Per Trade  : "
        f"{_format_money(stats.expectancy)}"
    )
    print(
        f"Average Return        : "
        f"{stats.average_return_percent:.4f}%"
    )
    print(
        f"Average Holding Time  : "
        f"{stats.average_holding_minutes:.2f} minutes"
    )
    print(
        f"Longest Win Streak    : "
        f"{stats.longest_winning_streak}"
    )
    print(
        f"Longest Loss Streak   : "
        f"{stats.longest_losing_streak}"
    )
    print(
        f"Starting Equity       : "
        f"${stats.starting_equity:,.2f}"
    )
    print(
        f"Estimated Equity      : "
